...
// Symbol_List_Map::Symbol_List_Map () {
    ...
    // printf ("!!!!!!!!! \n");
    // asm volatile ("vmcall; \n\t");
    // // assert(m_listheader.get());
    // // assert(m_listtail.get());
    // // printf ("m_listhead: %p, m_listtail: %p. \n", m_listheader.get(), m_listtail.get());
    // // printf ("m_listhead: %p, m_listtail: %p. \n", m_listheader, m_listtail);
    // // printf ("m_listhead: %p, m_listtail: %p. \n", m_listheader.get(), m_listtail.get());

    // // exit(0);
    // // m_listheader.get()->next = m_listtail ;
    // // m_listtail.get()->prev = m_listheader ;
    ...
    // m_sm[ADDR_TO_KEY(m_listtail.get()->addr)] = m_listtail ;
    // exit(0);
}

Symbol_List_Map::~Symbol_List_Map () {
    //SymCellPtr tmp = m_listheader;
    m_sm.clear () ;
    // do {
    //     m_listheader = tmp->next ;
    //     delete (tmp.get()) ;
    //     tmp = m_listheader ;
    // } while (tmp != NULL) ;
}
...
bool Symbol_List_Map::GetAffectedCells (SymCellPtr &insertAfter, SymCellPtr &insertBefore, 
    ...
    // auto it = m_sm.find(header_start) ;
    auto it = m_sm.find(ADDR_TO_KEY(header_start)) ;
    
    // /***********/
    // SymCellPtr tmp ;
    // std::cout << std::hex << header_start << "::" << tail_end << std::endl ;
    // for(tmp=m_listheader; tmp!=m_listtail; tmp=tmp->next) {
    //     std::cout << std::hex << tmp->addr << "::" << tmp->size << std::endl ;
    // }
    // std::cout << std::hex << tmp->addr << "::" << tmp->size << std::endl  ;
    // /***********/
...
bool Symbol_List_Map::Merge(SymCellPtr src_list) {
